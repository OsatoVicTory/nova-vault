...
import NftBuyForm from './buyForm';
import SuccessModal from '../../components/modals/success';
import { AssetFile } from './assetFile';


const NftAssetPutForSale = () => {
    ...
    const errors = [0, "Wrong Gallery and/or Nft id in the URL.", "Invalid owner address in URL", "This Nft is not owned by this user", "This Nft is not for sale"];
    ...
    return (
        ...
        {forSale === true && <NftBuyForm closeModal={() => setForSale(false)} 
        successFn={() => setForSale("success")} owner={owner} token_id={token_id} nft={nft} />}

        {forSale === "success" && <SuccessModal closeModal={() => setForSale(false)} 
        text={"Successful purchase. The Nft is now yours."} />}

        </div>
    )
};

export default NftAssetPutForSale;