...
import "../asset/assetFile.css";
import { IoPauseSharp } from "react-icons/io5";
import { IoMdPlay } from "react-icons/io";
import { getVideoImage } from "../../utils";
import { Skeleton } from "../../components/loading";

export const AudioFileToUpload = ({ data }) => {
...
export const VideoFileToUpload = ({ data }) => {
    ...
    const getVideoImageUrl = async () => {
        try {
            const url = await getVideoImage(data.url);
            setThumbnail(url);
        } catch(err) {
            console.log(err);
        }
...
export const AssetFileToUpload = ({ data }) => {
    return (
        <>
            {
                data.file_type === "audio" 
                ?
                <AudioFileToUpload data={data} />
                :
                <VideoFileToUpload data={data} />
            }
        </>
    );
};