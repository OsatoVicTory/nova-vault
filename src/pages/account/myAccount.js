...
import { 
    ...
    createNftSubmitContractInstance, divideBigDecimals, parseBigInt 
} from "../../services/creators";
import { fetchAcceptedNFTs, fetchCollectedNFTs, fetchOwnedNFTs } from "../../services/nfts";
import { FaCheck } from "react-icons/fa6";
import EditUserModal from "../signup/modal";
...
const MyAccount = () => {
    ...
    const fetchNft = async (val, nftLibraryContractInstance, nftSubmitContractInstance) => {
        ...
        const res = { 
            ...parseNftMetaData(nft_data), gallery_id: String(gallery_id), creator,
    ...
    const getRouteObject = useCallback((val) => {
        ...
        } else { // collected or for sale, here in user account should route to for sale, only in another user should collected route to collected
            return {
                pathname:  `/app/asset-sell/${val.nft_library_id}/${val.gallery_id}/${val.token_id}/${user_address}`,
                state: { nft: { ...val } } 
            }
        }
    }, [route]);
    ...
    return (
        <div className="Account w-full">
                ...
                <div className="ah-sticky w-full">
                    {!accountLoading && <div className={`ah-display-on-scroll ${scrolled} w-full`}>
                        <div className="ados">
                            ...
                            <div>
                                <button className="profile-btn pointer" onClick={() => setEditUser(true)}>
            ...
            {editUser && <EditUserModal closeModal={() => setEditUser(false)} />}
        </div>
    );
};
